...
function policyFor(role: string, permissions: Record<string, boolean>): SpecialtiesPolicy {
  const policy = new SpecialtiesPolicy();
  policy.setRoleResolver(jest.fn().mockResolvedValue({ permissions: { specialties: permissions } } as unknown as Role));
