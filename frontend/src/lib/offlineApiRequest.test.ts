...
describe("offlineApiRequest mutation queueing", () => {
  ...
  it("computes dashboard metrics from the cached products/sales lists in local install mode", async () => {
    const storage = new Map<string, string>();
    vi.stubGlobal("localStorage", {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
    ...
    setInstallMode("local");
    // offlineApiRequest first tries getCachedResponse(url, "dashboard")
    // itself (never populated, must resolve falsy) before falling into the
    // dashboard-specific branch, which then asks for the products and sales
    // lists by their own cache keys - respond by argument, not call order.
    vi.mocked(getCachedResponse).mockImplementation(async (_key, collection) => {
      if (collection === "products") {
        return [
          { id: "p1", name: "Widget", stocks: { quantity: 1 }, minStockAlert: 5 },
        ];
      }
      if (collection === "sales") {
        return [{ id: "s1", total: "9.99", createdAt: new Date().toISOString() }];
      }
      return null;
    });
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      lowStockItems: 1,
      todaysSales: "9.99",
      recentSales: [expect.objectContaining({ id: "s1" })],
      lowStockAlerts: [expect.objectContaining({ id: "p1" })],
    });

    // This test's mockImplementation would otherwise persist and answer
    // every later test's getCachedResponse call too.
    vi.mocked(getCachedResponse).mockReset();
  });

  it("computes day-grouped sales analytics from the cached sales list in local install mode", async () => {
    const storage = new Map<string, string>();
    vi.stubGlobal("localStorage", {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
    ...
    setInstallMode("local");
    // Only answer the actual sales-list cache key - offlineApiRequest also
    // makes its own getCachedResponse(url, "sales") call for the raw
    // analytics URL first, which must stay uncached (null) so it falls
    // through into the analytics-specific branch instead of short-circuiting.
    vi.mocked(getCachedResponse).mockImplementation(async (key, collection) => {
      if (collection !== "sales" || key !== "/api/sales/tenant-1") return null;
      return [
        {
          id: "s1",
          createdAt: new Date().toISOString(),
          items: [{ quantity: 2, unitPrice: "10.00", product: { cost: "4.00" } }],
        },
      ];
    });
    ...
    const response = await offlineApiRequest(
      "GET",
      "/api/sales/analytics/tenant-1?dateRange=7d",
      undefined,
      { collection: "sales" }
    );
    ...
    expect(fetch).not.toHaveBeenCalled();
    await expect(response.json()).resolves.toEqual([
      expect.objectContaining({ sales: 2, revenue: "20.00", cost: "8.00", profit: "12.00" }),
    ]);
    vi.mocked(getCachedResponse).mockReset();
  });

  it("filters the cached sales list by date range for the sales report view", async () => {
    const storage = new Map<string, string>();
    vi.stubGlobal("localStorage", {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
    ...
    // Only answer the actual sales-list cache key, not the report URL's own
    // (necessarily uncached) getCachedResponse(url, "sales") lookup.
    vi.mocked(getCachedResponse).mockImplementation(async (key, collection) => {
      if (collection !== "sales" || key !== "/api/sales/tenant-1") return null;
      return [
        { id: "in-range", createdAt: today.toISOString() },
        { id: "out-of-range", createdAt: longAgo.toISOString() },
      ];
    });
    ...
    const response = await offlineApiRequest(
      "GET",
      `/api/sales/tenant-1/report/${start.toISOString()}/${end.toISOString()}`,
      undefined,
      { collection: "sales" }
    );
    ...
    expect(fetch).not.toHaveBeenCalled();
    await expect(response.json()).resolves.toEqual([
      expect.objectContaining({ id: "in-range" }),
    ]);
    vi.mocked(getCachedResponse).mockReset();
  });

  it("returns the raw sales list for a single product (not aggregated), tagged 'sales'", async () => {
    const storage = new Map<string, string>();
    vi.stubGlobal("localStorage", {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
    ...
    setInstallMode("local");
    const older = { id: "older", createdAt: new Date(Date.now() - 86400000).toISOString(), items: [{ productId: "p1" }] };
    const newer = { id: "newer", createdAt: new Date().toISOString(), items: [{ productId: "p1" }] };
    const other = { id: "other", createdAt: new Date().toISOString(), items: [{ productId: "p2" }] };
    vi.mocked(getCachedResponse).mockImplementation(async (key, collection) => {
      if (collection !== "sales" || key !== "/api/sales/tenant-1") return null;
    ...
    const response = await offlineApiRequest(
      "GET",
      "/api/sales/product/p1?tenantId=tenant-1",
      undefined,
      { collection: "sales" }
    );

    expect(fetch).not.toHaveBeenCalled();
    await expect(response.json()).resolves.toEqual([newer, older]);
    vi.mocked(getCachedResponse).mockReset();
  });

  it("computes per-product analytics from the cached sales list, tagged 'products' not 'sales'", async () => {
    const storage = new Map<string, string>();
    vi.stubGlobal("localStorage", {
      getItem: (key: string) => storage.get(key) ?? null,
      setItem: (key: string, value: string) => storage.set(key, value),
      ...
      return [
        {
          ...
          createdAt: new Date().toISOString(),
          items: [
            { productId: "p1", quantity: 3, unitPrice: "5.00", product: { cost: "2.00" } },
            { productId: "p2", quantity: 9, unitPrice: "1.00", product: { cost: "0.10" } },
          ],
        },
      ];
    });
    ...
    const response = await offlineApiRequest(
      "GET",
      "/api/products/analytics/p1?dateRange=7d&tenantId=tenant-1",
      undefined,
      { collection: "products" }
    );
    ...
    expect(fetch).not.toHaveBeenCalled();
    await expect(response.json()).resolves.toEqual([
      expect.objectContaining({ sales: 3, revenue: "15.00" }),
    ]);
    vi.mocked(getCachedResponse).mockReset();
  });
